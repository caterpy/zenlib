...
def draw_to_axes(view, **kwargs):
    ...
    **Returns**:
    ...
    if len(kwargs) > 0:
        raise ZenException('Unexpected keyword arguments: %s' % ','.join(list(kwargs.keys())))
    ...
    for nidx in G.nodes_iter_():
        ...
        if shape == CIRCLE:
            ax.add_patch(Circle(
                (x, y), radius=dim[0], facecolor=color, edgecolor=bcolor, linewidth=bwidth, zorder=2))
        else:
    ...
    for eidx in G.edges_iter_():
        ...
        ax.add_line(Line2D((pos[x, 0], pos[y, 0]), (pos[x, 1], pos[y, 1]),
                           linewidth=ewidth, color=ecolor, alpha=alpha, zorder=1))
