    ...
    def test_deven(self):
        ...
        for N in xrange(190, 200):
            ...
            for Gf, lbl in [(lambda: erdos_renyi(N, float(k)/N, directed=True), 'ER'),
                            ...
                            (lambda: local_attachment(
                                N, k, max(int(.25*k), 1)), 'LA, r=.25'),
                            (lambda: local_attachment(
                                N, k, max(int(.5*k), 1)), 'LA, r=.5'),
...
class DiGraphTestCase(unittest.TestCase):
    ...
    def test_add_bad_nobjs(self):
        ...
        try:
            G.add_node("x")
            self.fail(
                'adding a second node with the same node object should have raised an exception')
        except ZenException:
            pass
    ...
    def test_add_node_x_error(self):
        ...
        try:
            G.add_node_x(idx, G.edge_list_capacity,
                         G.edge_list_capacity, None, None)
