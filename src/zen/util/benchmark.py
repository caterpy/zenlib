...
class Benchmark:
    ...
    def __load_benchmarks(self):
        ...
        bm_names = [x[3:] for x in [x for x in list(methods.keys()) if x.startswith('bm_')]]
    ...
    def run(self):
        ...
        print ('\n{}:'.format(self.name))
        for name, fxns in list(self.tests.items()):
            run_times = []
            print ('\tTest: {}'.format(name.ljust(self.max_name_len+2, ' ')), end='')
    ...
    def plot_bars(self, order=None, raw=False):
        ...
        width = 0.8
        pylab.bar(list(range(len(data))), data, width=0.8)
        ...
        pylab.xticks([x+width/2.0 for x in range(len(data))],
                     names, fontsize=18, fontweight='bold')
