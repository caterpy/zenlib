...
def _draw_boundary(scale=1.0, linewidth=2.0, color='black'):
    ...
    pyplot.plot([0-e/1.25, scale+e/1.25, scale / 2, 0-e/1.25], [0-e/2.2, 0-e /
                                                                2.2, math.sqrt(3.0*(scale+e)**2) / 2, 0-e/2.2], color, linewidth=linewidth)
...
def _plot(t, color=None, marker=None, markersize=8.0, mfc='none', mec=None, mew=1.0, label=None, picker=None):
    ...
    if color and marker:
        if mfc == 'none' and mec is None:
            mec = color
        pyplot.plot(xs, ys, color=color, marker=marker, mew=mew, mec=mec, mfc=mfc,
                    mfcalt='none', markersize=markersize, rasterized=False, label=label, picker=picker)
    else:
...
def _heatmap_scatter(pts, steps):
    ...
    for (x, y, z) in pts:
        ...
        if i == steps-1:
            ...
            # horizontal index
            # 'divide' by half the width of a subtriangle
            k = math.trunc((xt - float(i)*(1./steps/2.))*steps*2)
            ...
            if k == 0:
                ...
                # find the part of xt to the right of the k-th triangle half-width, normalized
                xtt = (xt - float(k)*(1./steps/2.))/(1./steps/2.)
                # find height within the i-th row, normalized
                ytt = (yt - float(i)*(_SQRT3OVER2/steps))/(_SQRT3OVER2/steps)
...
# heatmap=False, color='b', marker='o', markersize=8.0, mfc='none', mec=None, mew=1.0, label=None, picker=None, num_steps=15, cmap=None):
def _plot_profiles(items, **kwargs):
    """
    ...
    if heatmap:
        ...
        for p in pts:
            _plot(p, color=color, marker=marker, markersize=markersize,
                  mfc=mfc, mec=mec, mew=mew, label=label, picker=picker)
